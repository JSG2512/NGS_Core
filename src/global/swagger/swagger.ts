...
export function setupSwagger(app: INestApplication): void {
  const options = new DocumentBuilder()
    .setTitle('NAK GOP SAE API')
    .setDescription('NAK GOP SAE API 문서입니다.')
    .setVersion('1.0.0')
    .addBearerAuth(
      {
        type: 'http',
    ...
    .addServer('https://dev.dahyeon.us', '프로덕션 서버')
    .addServer('https://dev.dahyeon.us', '개발 서버')
    .build();
  ...
  const customOptions: SwaggerCustomOptions = {
    customCss: `
      ...
      .swagger-ui table tbody tr td:first-of-type { padding: 30px 0 0 0 }
      .swagger-ui table tbody tr td { padding: 30px 0 0 0 }
      .swagger-ui .markdown code, .swagger-ui .renderedMarkdown code { font-family: 'NanumSquareRound', 'sans-serif'; font-weight: 700; font-size: 15px; color: #004eff }
      `,
    customSiteTitle: 'Life Calendar API Docs',
  };
